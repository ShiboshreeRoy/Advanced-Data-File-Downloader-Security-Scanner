...
CRAWLED_PAGES_DIR = os.path.join(DOWNLOAD_DIR, "crawled_pages")
...
def print_banner():
 ...
 |_____/ \__,_|\__,_|_|  \__,_| |_____/ \___|_|    \_/ \___|_|  |___/\___|
          ...
          Security Scanner v0.9.0 - Advanced Data & File Downloader Dev by Space-Exe
""" + RESET)
...
def crawl_website(start_url, max_pages=50):
    """
    Recursively crawl and download HTML pages from the website.
    Only pages within the same domain as start_url are downloaded.
    """
    print(YELLOW + "[*] Starting website crawl..." + RESET)
    visited = set()
    pages = {}
    ...
    while queue and len(visited) < max_pages:
        ...
        try:
            ...
            if response.status_code == 200 and "text/html" in response.headers.get("Content-Type", ""):
                ...
                pages[url] = file_path
                print(GREEN + f"[✔] Crawled: {url}" + RESET)
                # Parse for new links
                soup = BeautifulSoup(response.text, "html.parser")
                for link in soup.find_all("a", href=True):
                    ...
                    if parsed_link.netloc == domain:
                        ...
                        normalized = parsed_link.scheme + "://" + parsed_link.netloc + parsed_link.path
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CRAWLED_PAGES_DIR):
        os.makedirs(CRAWLED_PAGES_DIR)
    ...
    # Perform website crawling (download all pages)
    crawled_pages = crawl_website(target_url, max_pages=50)
